...
//         [
//             'store_name' => null,
//             'enabled_currencies' => ['RUB', 'USDT'] // Default currencies
//         ]
//     );
    
//     // Get enabled currencies from profile
//     $enabledCurrencies = $profile->enabled_currencies ?? ['RUB', 'USDT'];
...
public function index(Request $request)
{
    ...
    // If no user_code, redirect to code entry
    if (!$userCode) {
        return view('enter-code');
    }
    ...
    // Calculate totals DYNAMICALLY for enabled currencies
    $totalOwedByMe = [];
    $totalOwedToMe = [];
    $totalExpenses = [];
    $totalSafekeeping = [];
    ...
    foreach ($enabledCurrencies as $currency) {
        $totalOwedByMe[$currency] = $owedByMe->where('currency', $currency)->sum('amount');
        $totalOwedToMe[$currency] = $owedToMe->where('currency', $currency)->sum('amount');
        $totalExpenses[$currency] = $expenses->where('currency', $currency)->sum('amount');
        $totalSafekeeping[$currency] = $safekeeping->where('currency', $currency)->sum('amount');
        $debtBalance[$currency] = $totalOwedToMe[$currency] - $totalOwedByMe[$currency];
    }
    ...
    return view('index', compact(
        'owedByMe', 'owedToMe', 'expenses', 'safekeeping',
        'totalOwedByMe', 'totalOwedToMe',
        'totalExpenses', 'totalSafekeeping',
        'debtBalance',
        'enabledCurrencies',
        'activeTab',
        'profile'
    ...
    public function storeDebt(Request $request)
{
    $request->validate([
        ...
        'currency' => 'required|in:RUB,USDT,LYD'
    ]);
